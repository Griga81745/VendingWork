...
import {
  ...
  Fade,
  MenuItem,
  withStyles, ExpansionPanel, ExpansionPanelSummary, ExpansionPanelDetails
...
const RadioTest = () => {
  ...
  // should run on mount, but only runs on update
  useEffect(() => {
    getRadioShows();
  }, [date]);
          ...
          <div className="kt-portlet__head-toolbar">
            ...
            <div className="d-none">
              <SingleDatePicker
                ...
                // isOutsideRange={
                //     (day) => isInclusivelyBeforeDay(day, dateStartMin) || isInclusivelyAfterDay(day, dateStartMax)
                // }
                onDateChange={date => {
                  setDate(date);
                ...
                >
                  ...
                  {!!dataShows[data.blocks[key].hour] &&
                    ...
                    Object.keys(dataShows[data.blocks[key].hour]).map(key2 => {
                      if (
                        !!data.campaigns[
                          dataShows[data.blocks[key].hour][key2]["campaign_id"]
                        ]
                      ) {
                        return (
