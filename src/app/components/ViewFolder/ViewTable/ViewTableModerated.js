...
import SlideModerated from "./SlideModerated";
import SlideModeratedRadio from "./SlideModeratedRadio";
...
const ViewTableModerated = (props) => {
  const {
    data,
    screens,
    currentUser,
  ...
  useEffect(() => {
    if (addFile) {
      ...
      console.log("fffffffffffgggggggggggggggg5555")
  ...
  function stableSort(array, comparator) {
    ...
    console.log(Object.keys(array));
    console.log(
      Object.keys(array).slice(
        props.currentPage * 20,
        props.currentPage * 20 + 20
      )
    );
  ...
  const activeF = async (data, status) => {
    if (data == null) {
      data = selected;
    }
    try {
  ...
  return (
    <>
      <Hidden only={["xs", "sm"]}>
        <div
          className={`fontGR mx-4 helpSelect align-items-center mt-3 c274C77 fS14 ${
            isSelectedLineMenu() ? "d-flex" : "d-none"
          }`}
        >
          <Button
      ...
      <Hidden only={["md", "lg", "xl"]}>
        <div
          className={`w-100 fontGR helpSelect2 align-items-center mt-3 c274C77 fS14 ${
            isSelectedLineMenu() ? "d-flex" : "d-none"
          }`}
        >
          <Button
          ...
          <Tooltip title="Delete" className={"ml-auto"}>
            <IconButton
              size="small"
              className={"helpButton small c274C77"}
              onClick={() => setSelected({})}
              className={"p-2"}
            >
              <CloseIcon />
        ...
        <Table>
          ...
          <TableBody>
            {!!Object.keys(boards).length &&
              stableSort(boards, getComparator(order, orderBy)).map((keyyy) => {
                return (
                  <Row
                    activeF={activeF}
                    data={props.data}
...
export default withRouter(connect(mapStateToProps)(ViewTableModerated));
      ...
      <TableRow className={classes.rootTableRow}>
        ...
        >
          <div className={"d-flex align-items-center"}>
            ...
            <Avatar
              onClick={() => {
                console.log("sdfdsfdfdsfdseeeeeeeeeeeeeeeeeeeeeeee");
                props.setSetting({
                  ...props.setting,
                  togglerLightBox: true,
                  file_path:
                    process.env.REACT_APP_MY_HOST_CREATIVE +
                    `/creative/${data.creatives[keyyy].group_id}/${data.creatives[keyyy].link}`,
                });
              }}
              variant="rounded"
              src={
                process.env.REACT_APP_MY_HOST_CREATIVE +
                `/creative/${data.creatives[keyyy].group_id}/thumb${
                  data.creatives[keyyy].link.split(".")[0]
                }.${
                  data.creatives[keyyy].link.split(".")[1] == "jpeg"
                    ? "jpeg"
                    : "jpg"
                }`
              }
              className="mr-3 ml-2 cursor-pointer filePrevIcon"
            />
            <div className={"c448AFF"}>{data.creatives[keyyy].title}</div>
          </div>
        </TableCell>
        <TableCell align="right" padding="none" className={"fontGR"}>
          {data.creatives[keyyy].resw}x{data.creatives[keyyy].resh}
        </TableCell>
        <TableCell align="right" className={"fontGR"}>
          {data.creatives[keyyy].filesize !== 0
            ? (data.creatives[keyyy].filesize / 1024 / 1024).toFixed(2)
            : "-"}
        </TableCell>
        <TableCell align="right" padding="none" className={"fontGR"}>
          <Moment format="DD.MM.YYYY">
            {data.creatives[keyyy].created_at !== 0
              ? data.creatives[keyyy].created_at
              : "-"}
          </Moment>
        </TableCell>
        ...
        >
          <Grid item md={1} classes={{ root: classes.dataContainer }}>
            <Box
              className="fontGM text-nowrap"
              style={{ color: isStatus(boards[keyyy].status) }}
            >
              <FormattedMessage id={"STATUS." + boards[keyyy].status} />
            </Box>
          </Grid>
        </TableCell>
        <TableCell align="right" className={"px-2"}>
          <div className={"d-flex"}>
            {boards[keyyy].status != "running" && (
              <div
                className={"mr-4 c448AFF text-nowrap cursor-pointer"}
                onClick={() => props.activeF({ all: [+keyyy] }, "running")}
              >
                Одобрить
              </div>
            )}
            {boards[keyyy].status != "moder" && (
              <div
                className={"mr-4 c448AFF text-nowrap cursor-pointer"}
                onClick={() => props.activeF({ all: [+keyyy] }, "moder")}
              >
                Вернуть на модерацию
              </div>
            )}
            {boards[keyyy].status != "reject" && (
              <div
                className={"c448AFF text-nowrap cursor-pointer"}
                onClick={() => props.activeF({ all: [+keyyy] }, "reject")}
              >
                Отклонить
              </div>
            )}
        ...
        return (
          ...
          >
            ...
            >
              <Box
                className="fontGM text-nowrap"
                style={{ color: isStatus(boards[keyyy]["B" + keyB].status) }}
              >
                <FormattedMessage
                  id={"STATUS." + boards[keyyy]["B" + keyB].status}
                />
              </Box>
            </TableCell>
            <TableCell align="right" className={"px-2"}>
              <div className={"d-flex"}>
                {boards[keyyy]["B" + keyB].status != "running" && (
                  <div
                    className={"mr-4 c448AFF text-nowrap cursor-pointer"}
                    onClick={() =>
                      props.activeF({ [keyB]: [+keyyy] }, "running")
                    }
                  >
                    Одобрить
                  </div>
                )}
                {boards[keyyy]["B" + keyB].status != "moder" && (
                  <div
                    className={"mr-4 c448AFF text-nowrap cursor-pointer"}
                    onClick={() => props.activeF({ [keyB]: [+keyyy] }, "moder")}
                  >
                    Вернуть на модерацию
                  </div>
                )}
                {boards[keyyy]["B" + keyB].status != "reject" && (
                  <div
                    className={"c448AFF text-nowrap cursor-pointer"}
                    onClick={() =>
                      props.activeF({ [keyB]: [+keyyy] }, "reject")
                    }
                  >
                    Отклонить
                  </div>
