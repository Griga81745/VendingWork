...
import {
    ...
    Typography,
    Grid,
    makeStyles, Accordion, AccordionDetails, AccordionSummary,
    withStyles,
    Box, Tooltip,
    Button, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Backdrop,
    Drawer, CircularProgress, FormControl,
    IconButton,
    TextField, MenuItem, RadioGroup, FormControlLabel, Radio, FilledInput, InputAdornment, Input, ClickAwayListener,
    Popper, Grow, MenuList, Checkbox, Slider, Hidden, AppBar, Tab as TabM, Divider, Menu, ListItem, ListItemText
...
import { validateEmail } from "../../../app/utils/index";
...
import DisplayCheckUp from "./DisplayCheckUp";
...
import CreativesAccordion from './CreativeAccordion';
        ...
        <CreateCamp.Provider value={{
            ...
            {setting.checkUpPage == false &&
                <div className={'row nol mx-0'}>
                    ...
                    <div className={`tranz m-0 p-0 col-md-6 col-lg-7 col-xl-7 col-12  ${classes.mapBlock} ${!!showBlockMapV ? classes.showBlockMap : ""} `} >
                        <div className={'sticky-column'}>
                            <div className={`${classes.MapWidget} mt-3`}>
                                <Paper elevation={2} className={`d-flex align-items-center ${classes.MapWidgetH} px-3`}>
                                    <LightTooltip title="Доступно экранов" aria-label="add" placement="bottom" >
                                        <Box component={'span'} className={'d-flex cursor-pointer align-items-center ml-auto'}>
                                            <LocationOnIcon style={{ color: '#004fff' }} /><Box fontWeight={500} fontSize={20} >{Object.keys(boards).length}</Box>
                                        </Box>
                                    </LightTooltip>
                                    {Object.keys(allboards).length != Object.keys(boards).length &&
                                        <LightTooltip title="Недоступные экраны" aria-label="add" placement="bottom" >
                                            <Box component={'span'} className={'d-flex cursor-pointer align-items-center ml-4'}>
                                                <LocationOffOutlinedIcon style={{ color: '#ff5252', marginLeft: 10 }} /><Box className={'ml-1'} fontWeight={500} fontSize={20} >{Object.keys(allboards).length - Object.keys(boards).length}</Box>
                                            </Box>
                                        </LightTooltip>
                                    }

                                    {!!Object.keys(choosedBoards).length &&
                                        <LightTooltip title="Выбранные экраны" aria-label="add" placement="bottom" >
                                            <Box component={'span'} className={'d-flex cursor-pointer align-items-center ml-4'}>
                                                <VideoLabelOutlinedIcon style={{ color: '#004fff', marginLeft: 10 }} /><Box className={'ml-1'} fontWeight={500} fontSize={20} >{Object.keys(choosedBoards).length}</Box>
                                            </Box>
                                        </LightTooltip>
                                    }
                                </Paper>
                            ...
                            {!!choosedBoards.length &&
                                <div className={'cardMap cardMapBoard'}>
                                    <Typography>Выбрано конструкций <div className={'big d-inline'}>{choosedBoards.length}</div></Typography>
                                </div>
                            }
                            <DevicesMap key={"crateCampMap"} boards={boards} filterBoards={boards} choosedBoards={choosedBoards} showBlockMapV={showBlockMapV} />
                        </div>
                    </div>
...
function ErrorDialog(props) {
    ...
    return (
        <Dialog
            open={!!setting.openErrorDialog || !!setting.openErrorDialogTitle}
            onClose={() => {
        ...
        >
            <DialogTitle id="alert-dialog-title">{setting.openErrorDialogTitle}</DialogTitle>
            <DialogContent>
                <DialogContentText id="alert-dialog-description">
                    {setting.openErrorDialog}
                </DialogContentText>
            </DialogContent>
            <DialogActions>
                <Button onClick={() => {
