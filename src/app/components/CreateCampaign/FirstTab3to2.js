...
import LocationDeviceBoard from "./LocationDeviceBoard";
...
const FirstTab3to2 = (props) => {

  const { user, boards, page, choosedBoards, updateChoosedBoards, setting } = props;
  ...
  const isSelected = id => !!props.choosedBoards[id];
  ...
  return (
    <>
      {Object.keys(boards).map((key, index) => {
        //if(key.indexOf('g') == -1 && ( props.setting.typeShows == 1 || props.setting.typeShows == 2 ) ){
        //  if( boards[key].user_id != props.currentUser.id && props.currentUser.id != 72 && ( props.setting.typeShows == 1 || props.setting.typeShows == 2 )){
        //      return;
        //  }
        // }
        //console.log(boards[key])

        if (indexBoards > page * 10 - 1 || indexBoards < page * 10 - 10) {
          indexBoards++;
          return;
        }
        indexBoards++;
        ...
        const checkedB = choosedBoards[boards[key].id];
        return (
          <LocationDeviceBoard
            key={'boardList' + boards[key].id}
            freeShowsAllboards={props.freeShowsAllboards}
            boardData={boards[key]}
            checkedB={checkedB}
            filter={props.filter}
            isItemSelected={isItemSelected}
            create_file_paths={create_file_paths}
...
export default injectIntl(withRouter(connect(mapStateToProps)(FirstTab3to2)));
