    ...
    <div className="kt-form">
      <div className="kt-wizard-v2__content">
        <div className="kt-form__section kt-form__section--first">
          <div className="kt-wizard-v2__form">
            <div className={classes.header} >Медиаматериалы</div>
            <div className={classes.headerS} style={{ 'padding': "0 0 10px 0" }}>Загрузите файлы для вашей рекламной кампании</div>
            <div className={classes.headerS2} style={{ 'padding': "0 0 2px 0" }}>Типы файлов: *.jpg *.mp4</div>
            <div className={classes.headerS2} style={{ 'padding': "0 0 2px 0" }}>Максимальный размер файла: 30mb</div>
            <div className={classes.headerS2} style={{ 'padding': "0 0 15px 0" }}>Максимум: 15 файлов</div>
          </div>
        </div>
      </div>

      {!!resolutions.length &&
        Array.from([...new Set(resolutions)]).map((i, ndx) => {
          return (
            <Card classes={{ root: classes.elevation }} className={`сardHeader ${setting.tabMedia == i ? 'active' : ''}`} onClick={() => {
              updateSetting({ ...setting, tabMedia: i })
            }} >
              <CardHeader
                avatar={
                  <SettingsSVG />
      ...
      <Grid style={{ 'text-align': 'right' }}>
        <button className={`nextButton btn fw500`}
          onClick={() => props.stepLinksValidate(5)}
        >
          Далее
        </button>
      </Grid>
...
export default injectIntl(withRouter(connect(mapStateToProps)(FirstTab4)));
