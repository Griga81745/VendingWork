...
const AlbumViewDesktop = props => {
  const { classes, option, getBoardInfo, id, type, currentUser, setSetting, setting} = props;
              ...
              <Avatar
                onClick={() => {
                        setSetting({
                            ...
                            file_path: option.id == 'default' ? option.path_url+option.link : `https://st.de4.ru/boards/${id}/${option.link}`
                        })
                    }
                }
                variant="rounded"
                src={`${option.id == 'default' ? option.path_url : `https://st.de4.ru/boards/${id}/`}${option.id == 'default' ? "" : "thumb"}${option.link.split(".")[0]}.${option.link.split(".")[1] == "jpeg" ? "jpeg" : "jpg"}`}
