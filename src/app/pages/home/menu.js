...
import { Divider, makeStyles, Fade, MenuItem, Menu as MenuD, withStyles, Tooltip } from "@material-ui/core";
...
const Menu = props => {
  ...
  const deleteCompany = async id => {
    ...
    try {
      ...
      let res = await axios.post(
        `https://test.de4.ru/campaign/delete?campaign_id=${id}&subuser=${JSON.parse(localStorage.getItem("currentUser")).id}`
      );
  ...
  return (
    <>
      <button className="btn btn-icon btn-sm btn-icon-md" onClick={handleClick7}>
      ...
      >
        <MenuItem onClick={handleClose7} classes={{ root: classes.menuItem }}>
          Статистика
        </MenuItem>
        <MenuItem onClick={handleClose7} classes={{ root: classes.menuItem }}>
          Загрузить слайд
        ...
        <Divider />
        {props.type !== "devices" && (
          <MenuItem classes={{ root: classes.menuItem }} onClick={() => preDelete(props)}>
            Остановить
          </MenuItem>
        )}
      </MenuD>
