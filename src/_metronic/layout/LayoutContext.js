...
function findPageConfig(currentPage, items, breadcrumbs) {
  ...
  for (const item of items) {
    ...
    // if ( currentPage.substring(0, currentPage.lastIndexOf('/')) === item.page || currentPage === item.page && !item.submenu) {
    console.log(currentPage.split("/").length);

    if (
      (currentPage.substring(0, currentPage.lastIndexOf("/")) === item.page &&
        currentPage.split("/").length > 2) ||
      ("wizard" === item.page && currentPage.split("/").length > 1) ||
      currentPage === item.page
    ) {
      // console.log('hhhhhh');
      // console.log(currentPage.substring(0, currentPage.lastIndexOf('/')));
      //console.log('ggggg');
      return item;
    }
...
function init({ pathname, menuConfig }) {
  ...
  if (pageConfig) {
    ...
    state.subheader.subtitle = pageConfig.subtitle;
  }
